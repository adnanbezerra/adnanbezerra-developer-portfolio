...
import { ArrowRight, Code2, Database, Layout, Server, Terminal, Github, Linkedin, Mail, ExternalLink, Send } from "lucide-react";
  ...
  const stackCategories = [
    {
      ...
      items: ["Node.js", "Express", "NestJS", "Fastify", "Spring Boot (Java)"]
    },
    {
      title: "Frontend",
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/30">
      ...
      <main className="relative z-10">
        ...
        {/* ABOUT SECTION */}
        <section id="about" className="py-24 bg-card/30 border-y border-white/5 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="flex items-center gap-4 mb-12">
                <h2 className="text-3xl md:text-4xl font-bold font-display text-white">{t.about.title}</h2>
                <div className="h-[1px] flex-1 bg-white/10"></div>
              </div>
            </FadeIn>

            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
              <FadeIn delay={0.2} className="lg:col-span-7 space-y-6 text-muted-foreground leading-relaxed text-lg">
                <p>{t.about.p1}</p>
                <p>{t.about.p2}</p>
                <p>{t.about.p3}</p>
                ...
                <div className="pt-6">
                  ...
                  <div className="p-6 rounded-2xl bg-white/5 border border-white/10 relative overflow-hidden group">
                    ...
                    <p className="italic relative z-10 text-base">"{t.about.philosophyText}"</p>
                  </div>
                </div>
              </FadeIn>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="flex items-center gap-4 mb-4">
                <h2 className="text-3xl md:text-4xl font-bold font-display text-white">{t.stack.title}</h2>
                <div className="h-[1px] flex-1 bg-white/10"></div>
              </div>
        ...
        {/* PROJECTS SECTION */}
        <section id="projects" className="py-24 bg-card/30 border-y border-white/5 backdrop-blur-sm">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="flex items-center gap-4 mb-12">
                <h2 className="text-3xl md:text-4xl font-bold font-display text-white">{t.projects.title}</h2>
                <div className="h-[1px] flex-1 bg-white/10"></div>
              </div>
            </FadeIn>
              ...
              {t.projects.items.map((project, idx) => (
                <FadeIn key={idx} delay={0.1 * idx}>
                  <div className="group rounded-2xl p-8 glass-panel hover-glow h-full flex flex-col relative overflow-hidden">
                    ...
                    <span className="text-xs font-mono text-primary mb-4 tracking-wider uppercase">{project.role}</span>
                    <h3 className="text-2xl font-bold font-display text-white mb-4 group-hover:text-primary transition-colors">{project.name}</h3>
                    ...
                    <div className="flex flex-wrap gap-2 mt-auto pt-6 border-t border-white/10">
                      {project.tech.map((tech, i) => (
                        <span key={i} className="text-xs font-medium text-muted-foreground bg-background/50 px-2 py-1 rounded">
                          {tech}
                        </span>
                      ))}
                    </div>
        ...
        <section id="experience" className="py-24">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="flex items-center gap-4 mb-12">
                <h2 className="text-3xl md:text-4xl font-bold font-display text-white">{t.experience.title}</h2>
                <div className="h-[1px] flex-1 bg-white/10"></div>
              </div>
            </FadeIn>
            ...
            <div className="max-w-4xl mx-auto relative">
              ...
              {t.experience.items.map((item, idx) => (
                <FadeIn key={idx} delay={0.1 * idx} className="mb-12 relative">
            ...
            <FadeIn delay={0.4} className="mt-24">
              <div className="p-8 rounded-2xl bg-gradient-to-r from-primary/10 to-indigo-500/10 border border-primary/20 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                  <div>
                    <span className="text-xs font-mono text-primary mb-2 block uppercase tracking-wider">{t.publications.title}</span>
                    <h3 className="text-xl font-bold text-white mb-2">{t.publications.name}</h3>
                    <p className="text-muted-foreground">{t.publications.desc}</p>
                  </div>
                  <a 
        ...
        {/* CONTACT SECTION */}
        <section id="contact" className="py-24 bg-card/30 border-t border-white/5 backdrop-blur-sm">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <FadeIn>
              <div className="text-center mb-16">
                <h2 className="text-4xl md:text-5xl font-bold font-display text-white mb-6">{t.contact.title}</h2>
            ...
            <FadeIn delay={0.2}>
              <form onSubmit={handleSubmit(onSubmit)} className="glass-panel p-8 md:p-10 rounded-3xl space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-white/80">{t.contact.form.email}</label>
                    <input 
                      {...register("email")}
                      type="email"
